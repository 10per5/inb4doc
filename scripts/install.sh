...
#   <prefix>/editor/       <- read-only thin-shell copy
#
...
while [ "$#" -gt 0 ]; do
    ...
    --editor-root)
        [ "$#" -ge 2 ] || {
            echo "error: --editor-root requires a value" >&2
            usage
        }
        EDITOR_ROOT="$2"
        shift 2
...
case "$PREFIX" in
"" | "/")
    echo "error: refusing to operate on '$PREFIX'" >&2
    exit 1
    ;;
esac
...
# Accepts the flat predep layout (inb4doc-gui + editor/), plus archives wrapped
...
resolve_payload() {
    local base="$1"
    if [ -f "$base/inb4doc-gui" ] && [ -d "$base/editor" ]; then
        PAYLOAD_BIN="$base"
...
obtain_payload() {
    ...
    http://* | https://*)
        ...
        if ! resolve_payload "$TMP_DIR"; then
            echo "error: no usable payload in archive (expected inb4doc-gui + editor/)" >&2
            exit 1
        fi
        ;;
    *.tar.gz | *.tgz | *.tar)
    ...
    *)
        if ! resolve_payload "$src"; then
            echo "error: no usable payload in '$src' (expected inb4doc-gui + editor/)" >&2
            exit 1
        fi
        ;;
    esac
...
verify() {
    ...
    [ -x "$PREFIX/inb4doc-gui" ] || {
        echo "FAIL: missing executable $PREFIX/inb4doc-gui" >&2
        return 1
    }
    [ -f "$PREFIX/editor/index.html" ] || {
        echo "FAIL: missing thin shell $PREFIX/editor/index.html" >&2
        return 1
    }
