...
**Editor** — A ProseKit WYSIWYG interface (Turbo + Stimulus). Loads and saves content via HTTP API calls (`GET/PUT/DELETE /content/*`, `GET /api/tree`, `POST /api/move`). The editor frontend is a static SPA — HTML + JS + CSS — with no server logic of its own.
...
| Editor     | ProseKit                           | ProseMirror-based, markdown-native, WYSIWYG and MD source, no HTML↔MD roundtrip |
