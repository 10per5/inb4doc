...
export async function mountProviderDialog(
  ...
  return new Promise((resolve) => {
    ...
    const currentInfo = providerBadges[currentProvider] || {
      icon: "❓",
    ...
    function renderBody() {
      ...
      const remoteAppFallback = inGuiMode
        ? !remoteAvailable
        : configuredHost === "app://" ||
          configuredHost === "app://_" ||
          configuredHost.startsWith("app://_/");
      const remoteGuiFallback =
        !remoteAvailable && !remoteAppFallback && inGuiMode;
      ...
      const mergedProviders = providers.map((p) => ({
        ...
        available:
          p.type === "remote"
            ? remoteAvailable || remoteAppFallback || remoteGuiFallback
            : p.available,
      ...
      const body = html`
        <div class="provider-dialog-body">
          <div class="provider-current">
            Currently using:
            <strong>${currentInfo.icon} ${currentInfo.label}</strong>
          </div>
          <div class="provider-options">
            ${mergedProviders.map(
              (p) => html`
                <div
                  class="provider-option ${p.available
                    ? ""
                    : "provider-option-disabled"} ${selectedType === p.type
                ...
                >
                  <div class="provider-option-icon">
                    ${providerBadges[p.type]?.icon || "❓"}
                  </div>
                  <div class="provider-option-info">
                    <div class="provider-option-name">
                      ${providerBadges[p.type]?.label || p.type}
                    </div>
                    <div class="provider-option-desc">${p.description}</div>
                    ${p.reason && p.type !== "remote"
                      ? html`<div
                          class="provider-option-reason ${(p as any)
                            .appFallback || (p as any).guiFallback
                            ? "info"
                            : ""}"
                        >
                          ${(p as any).appFallback || (p as any).guiFallback
                            ? "ℹ️"
                            : "⛔"}
                          ${p.reason}
                        </div>`
                      : ""}
                    ${p.type === currentProvider
                      ? html`<div class="provider-option-current-badge">
                          current
                        </div>`
                      : ""}
      ...
      render(
        html`
          <style>
            ...
            .provider-option:hover:not(.provider-option-disabled):not(
                .provider-option-active
              ):not(.provider-option-selected) {
    ...
    function renderRemoteSection() {
      ...
      function scheduleProbe() {
        ...
        probeTimer = setTimeout(() => {
          const host =
            (
              document.getElementById(hostId) as HTMLInputElement
            )?.value.trim() || "localhost";
          const port = parseInt(
            (document.getElementById(portId) as HTMLInputElement)?.value ||
              "3000",
            10,
          );
          ...
          connectionStore.probe().then(() => renderBody());
        }, 600);
      }
      ...
      const guiFallback =
        !connectionStore.remoteAvailable &&
        inGuiMode &&
        !appFallback &&
      ...
      const initialStatus = connectionStore.remoteAvailable
        ...
        : appFallback || guiFallback
          ? html`<div id="${statusId}" class="remote-status info">
              ℹ️ inb4doc-gui local API is used
            </div>`
          : inGuiMode
            ? html`<div id="${statusId}" class="remote-status"></div>`
            : html`<div id="${statusId}" class="remote-status err">
                Server unreachable
              </div>`;
