...
const STYLES = `
#inb4doc-external-overlay {
  position: fixed; inset: 0; background: rgba(0,0,0,0.4);
  z-index: 1000; display: flex; align-items: center; justify-content: center;
}
.inb4doc-external-window {
...
.inb4doc-external-header {
  padding: 1rem 1.5rem 0; font-size: 1.1rem; font-weight: 600; flex-shrink: 0;
  color: var(--color-text-primary);
}
.inb4doc-external-body {
  padding: 0.5rem 1.5rem; overflow-y: auto; flex: 1;
  color: var(--color-text-primary);
}
.inb4doc-external-footer {
  display: flex; gap: 0.5rem; justify-content: flex-end;
  padding: 0.75rem 1.5rem 1rem; flex-shrink: 0;
}
.inb4doc-external-message {
  margin: 0 0 0.75rem; color: var(--color-text-secondary); font-size: 0.9rem; line-height: 1.5;
}
.inb4doc-external-message strong { color: var(--color-text-primary); }
.inb4doc-external-toggle {
  display: block; width: 100%; padding: 0.4rem; margin-bottom: 0.5rem;
  border: 1px solid var(--color-border); border-radius: 4px; background: var(--color-bg-secondary);
  cursor: pointer; font-size: 0.85rem; color: var(--color-text-primary);
}
.inb4doc-external-toggle:hover { background: var(--color-bg-tertiary); }
.inb4doc-external-diff {
  border: 1px solid var(--color-border); border-radius: 4px;
  margin-bottom: 0.5rem; max-height: 300px; overflow-y: auto;
}
.inb4doc-external-diff-line {
...
function renderDiffHtml(diffLines: DiffLine[]): string {
  ...
  for (const line of limited) {
    ...
    html += `<div class="inb4doc-external-diff-line" style="background:${bg};color:${color}">${prefix}${escapeHtml(line.text)}</div>`;
  }
...
function buildOverlay(): HTMLDivElement {
  const existing = document.getElementById("inb4doc-external-overlay");
  if (existing) existing.remove();
  const el = document.createElement("div");
  el.id = "inb4doc-external-overlay";
  document.body.appendChild(el);
  return el;
}

function injectStyles(): void {
  if (document.getElementById("inb4doc-external-styles")) return;
  const style = document.createElement("style");
  style.id = "inb4doc-external-styles";
...
function buildDialog(path: string): DialogElements {
  ...
  window.className = "inb4doc-external-window";

  const header = document.createElement("div");
  header.className = "inb4doc-external-header";
  header.textContent = "File changed on disk";

  const body = document.createElement("div");
  body.className = "inb4doc-external-body";

  const msg = document.createElement("p");
  msg.className = "inb4doc-external-message";
  ...
  toggleBtn.className = "inb4doc-external-toggle";
  toggleBtn.textContent = "▸ View diff";
  body.appendChild(toggleBtn);

  const diffContainer = document.createElement("div");
  diffContainer.className = "inb4doc-external-diff";
  diffContainer.style.display = "none";
  body.appendChild(diffContainer);

  const footer = document.createElement("div");
  footer.className = "inb4doc-external-footer";
