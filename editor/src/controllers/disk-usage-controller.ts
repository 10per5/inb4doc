...
export default class DiskUsageController extends Controller {
  static targets = ["chartArea", "statsArea"];

  declare readonly chartAreaTarget: HTMLElement;
  declare readonly statsAreaTarget: HTMLElement;

  private diskUsageData: DiskUsageData | null = null;
  private currentMode: "dir" | "size" = "dir";
  ...
  toggleMode(event: Event): void {
    ...
    if (mode !== "dir" && mode !== "size") return;
    this.currentMode = mode;
    for (const b of this.element.querySelectorAll<HTMLElement>(".disk-usage-mode-btn")) {
  ...
  private renderChart(): void {
    if (!this.diskUsageData) return;
    const segments =
      this.currentMode === "dir"
        ? getDirSegments(this.diskUsageData)
        : getSizeSegments(this.diskUsageData);
    renderPieChart(this.chartAreaTarget, segments);
    renderStats(this.statsAreaTarget, this.diskUsageData, segments);
  }
}
